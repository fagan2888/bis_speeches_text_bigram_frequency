...
bigram_uniqueness_strength_dict_filepath = '/home/dmlab/jihye/GIT/bis_speeches_text_bigram_frequency/output/bigram-uniqueness-strength.pkl'
bigram_uniqueness_strength_dict = load_pkl(bigram_uniqueness_strength_dict_filepath)
...
def main():
    ...
    for printing_order in printing_order_dict.keys():
        for _period_category, _period_dict in bigram_uniqueness_strength_dict.items():
            wr.writerow([printing_order_dict[printing_order]])
            wr.writerow([_period_category, 'bigram', 'frequency', 'uniqueness', 'strength'])
